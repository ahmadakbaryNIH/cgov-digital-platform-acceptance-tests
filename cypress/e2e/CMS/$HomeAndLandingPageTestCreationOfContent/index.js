...
And('user enters {string} into Raw HTML Content text field under Guide Card Row', (value) => {
    cy.get("textarea[id^=edit-field-landing-contents-1-subform-field-guide-cards-1-subform-field-raw-html-0-value").type(value)
})
...
And('user selects {int} item from the media list', (num) => {
    cy.getIframeBody('iframe.entity-browser-modal-iframe').find("input[id^='edit-entity-browser-select-media894093']").eq(num - 1).check()
})
...
And('user selects {string} from List Item Style dropdown under list', (option) => {
    cy.get("select[name='field_landing_contents[1][subform][field_list_item_style]").select(option)
})
...
And('user selects {string} from List Item Style dropdown', (listItem) => {
    cy.get("select[name='field_landing_contents[1][subform][field_main_contents][1][subform][field_list_item_style]").select(listItem)
})

And('user fills out HTML Content text area with {string}', (value) => {
    cy.getIframeBody("iframe[title='Rich Text Editor, HTML Content field']", 1).find('p').type(value)
})

And('user enters {string} in Raw HTML Content under one column container', (value) => {
    cy.get("textarea[id^='edit-field-landing-contents-1-subform-field-main-contents-2-subform-field-raw-html-0-value']").type(value)
})

And('user enters {string} in Raw HTML Content under Two Column Container', (value) => {
    cy.get("textarea[id^='edit-field-landing-contents-2-subform-field-main-contents-0-subform-field-raw-html-0-value']").type(value)
})

And('user selects {string} from Source View dropdown', (sourceDropdown) => {
    cy.get("select[name='field_landing_contents[1][subform][field_main_contents][3][subform][field_source_view][0][target_id]']").select(sourceDropdown)
})

And('user selects {string} from Display dropdown', (dropdown) => {
    cy.get("select[name='field_landing_contents[1][subform][field_main_contents][3][subform][field_source_view][0][display_id]").select(dropdown)
})
...
And('user checks Include View Title checkbox', () => {
    cy.get("input[id^='edit-field-landing-contents-1-subform-field-main-contents-3-subform-field-source-view-0-options-title']").check()
})

And('user enters {string} into Items per page dropdown', (value) => {
    cy.get("input[id^='edit-field-landing-contents-1-subform-field-main-contents-3-subform-field-source-view-0-options-limit']").type(value)
})

And('user selects {string} from Pagination dropdown', (option) => {
    cy.get("select[id^='edit-field-landing-contents-1-subform-field-main-contents-3-subform-field-source-view-0-options-pager']").select(option)
})

And('user selects {string} from Card Theme dropdown', (Accent) => {
    cy.get("select[name='field_landing_contents[1][subform][field_card_theme]").select(Accent)
})

And('user selects {string} from Image Position dropdown', (dropdown) => {
    cy.get("select[name='field_landing_contents[1][subform][field_image_position]").select(dropdown)
})
...
And('user selects {int} Promotional Image from the list of images', (num) => {
    ...
    cy.get('input[name="field_landing_contents_1_subform_field_override_image_promotional_entity_browser_entity_browser"]').click({ force: true })
...
And('user selects {string} from Button Display dropdown', (dropdown) => {
    cy.get("select[name='field_landing_contents[1][subform][field_button_display]").select(dropdown)
})
